...
Invoke this skill by name (`to-phases`). It optionally accepts a `phase` argument
(a specific phase number to add or update); how arguments are passed depends on the
agent host.
...
3. Ask the user to confirm or modify phases
...
1. the `to-prd` skill creates `.phases/prd-*.md` (optional)
2. the `to-phases` skill creates `.phases/` structure (index.yaml + phase files)
3. the `go-team` skill (or node-team, zig-team) reads index.yaml to find current phase
...
- Using PLAN.md instead of `.phases/` structure (also avoids conflict with agent plan-mode files)
