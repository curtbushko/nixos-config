---
name: skill-creation
description: Use when creating new Agent Skills. Covers skill structure, YAML frontmatter, content organization, and testing skills with pressure scenarios.
---
...
to every supported agent host from a single canonical source:

- **Source of truth**: `modules/home/llm/skills/<skill-name>/SKILL.md` (in the nixos-config repo)
- **Deployed to**: each host's skills directory (e.g. `~/.claude/skills/` for Claude Code,
  `~/.codex/skills/` for Codex), symlinked by home-manager

Always create and edit skills in the nix repo, never in a deployed host directory.
Keep skills host-neutral: refer to other skills by name rather than by host path,
and use bundle-relative paths (e.g. `references/foo.md`) for files within a skill.

## Directory Structure

```
modules/home/llm/skills/
  skill-name/
