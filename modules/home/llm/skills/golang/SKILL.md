---
name: golang
description: Use when writing, testing, or reviewing Go code. Enforces TDD, hexagonal architecture with go-arch-lint, Cobra/Viper CLIs, testify, table-driven tests, and Taskfile-based build/lint/test quality gates. Includes references for architecture, BDD/Gherkin, protobuf, contract testing, and the Go Cloud SDK.
---
...
For structuring larger work into reviewable increments, use the portable planning
skills: the `to-prd` skill to capture requirements, the `to-phases` (or
`write-phase`) skill to break the PRD into `.phases/`, and a team skill (`go-team`)
to execute. That planning model is language-independent and documented in those
skills; the Go-specific concerns below are what you layer on top when the project
is Go.

When planning a Go feature, work each layer in dependency order (domain → ports →
application → adapters → wiring), specify behavior with BDD/Gherkin first (see
[references/bdd-gherkin.md](references/bdd-gherkin.md)), and size each task so it
covers one coherent concern with independently verifiable acceptance criteria, a
reviewable diff, and enough context for a fresh agent session to complete it.
