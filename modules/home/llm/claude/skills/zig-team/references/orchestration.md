...
This document defines the automated dispatch loop for the Zig Team skill.
...
|-------|--------------|----------|
| **Zig Builder** | `[[builder-context.md]]` | TDD workflow, Zig patterns, error handling, memory management, testing patterns |
| **Zig Reviewer** | `[[reviewer-context.md]]` | Memory safety checklist, error handling review, testing anti-patterns, output formats |
| **Task Manager** | (inline in template) | Module structure guidelines, task breakdown rules |
...
Task tool call:
  ...
  prompt: |
    ## Zig Builder: Implement Task {task.id} - {task.name}
    ...
    **Modify:**
    ...
    - Build Quality Gates (zig build, zig build test)
...
Task tool call:
  ...
  prompt: |
    ## Zig Reviewer: Spec Compliance - Task {task.id}
...
Task tool call:
  ...
  prompt: |
    ## Zig Reviewer: Code Quality - Task {task.id}
...
Task tool call:
  ...
  prompt: |
    ## Zig Builder: Fix Review Feedback - Task {task.id}
    ...
    **Priority {change.priority}:** {change.description}
    ...
    3. Ensure zig build && zig build test pass
...
if builder_output.blockers contains "compile error":
  # Include error output in next builder dispatch
```
...
4. Final validation (zig build, zig build test, zig fmt --check)
5. Summary report
```
